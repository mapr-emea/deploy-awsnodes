#! /bin/bash  
# CAUTION: This script must not exceed 16380 bytes!!
#
# PATRIC TODO: Test Ubuntu part
# PATRIC TODO: Check for errors during yum install and exit eventually
...
function install_openjdk_rpm() {
    echo "Installing OpenJDK packages (for rpm distros)" >> $LOG
        # JAMES modification to java version for bug in 3.0.1
...
function setup_mapr_repo_deb() {
    ...
    if [[ ${MAPR_VERSION} = "3.1.1" ]]; then
        MAPR_ECO="http://package.mapr.com/releases/ecosystem/ubuntu"
    elif [[ ${MAPR_VERSION} = "5.0.0" ]]; then 
        # new repo for 5.x ecosystem
        MAPR_ECO="http://package.mapr.com/releases/ecosystem-5.x/ubuntu"
    else    
        # new repo for 4.x ecosystem
        MAPR_ECO="http://package.mapr.com/releases/ecosystem-4.x/ubuntu"
    fi      
...
function setup_mapr_repo_rpm() {
    ...
    if [[ ${MAPR_VERSION} = "3.1.1" ]]; then
        ...
        # new repo for 5.x ecosystem
        MAPR_ECO="http://package.mapr.com/releases/ecosystem-5.x/redhat"
    else
        # new repo for 4.x ecosystem
        MAPR_ECO="http://package.mapr.com/releases/ecosystem-4.x/redhat"
    fi
